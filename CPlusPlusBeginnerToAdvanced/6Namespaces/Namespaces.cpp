// Bsp01.cpp : This file contains the 'main' function. Program execution begins and ends there.
//

#include <iostream>

namespace abc {
    void cout(std::string text) { std::cout << "===" << text << "===" << std::endl; }
    void cout(std::string text) { std::cout << "===" << text << "==="; }
}
...
using namespace xyz; // Was macht ein namespace f�r uns. Durch die Auslagerung haben wir die M�glichkeit Funkionsnamen mehrfach anzuwenden ohne eine Kollision aufzubauen.

int main()
{
    // Zur�ck zu den header Dateien und Prototypen. Zus�tzlich muss an die Dateien auch zu klassifizieren. Das hat super funktioniert.
    // Aber wenn man mehrere Dateien versucht auszulagern und die Dateine haben verschiedene Funktionalit�ten. Diese Gruppierung
    // versteht man erst wenn verschiedene namespaces erstellt wurden. 
    abc::cout("EIne �berschrift");
    xyz::cout("�berschrift2");

    cout("�berschrift2");
}

// ABER: Diese Gruppierung reicht uns leider nicht. K�nnte man Gruppen slbst weiter arbeiten lassen wollen und hier ist dann das Thema der Klassen aufgekommen.
...
The following example shows a namespace declarationand three ways that code outside the namespace can accesses their members.
...
In general, avoid putting using directives in header files(*.h) because any file that includes that header will bring everything in the namespace into scope, which can cause name hidingand name collision problems that are very difficult to debug.Always use fully qualified names in a header file.If those names get too long, you can use a namespace alias to shorten them. (See below.)

Declaring namespacesand namespace members
...
The std namespace
All C++ standard library typesand functions are declared in the std namespace or namespaces nested inside std.
...
#include <iostream>
    ...
    You can use inline namespaces as a versioning mechanism to manage changes to the public interface of a library.For example, you can create a single parent namespace, and encapsulate each version of the interface in its own namespace nested inside the parent.The namespace that holds the most recent or preferred version is qualified as inline, and is therefore exposed as if it were a direct member of the parent namespace.Client code that invokes the Parent::Class will automatically bind to the new code.Clients that prefer to use the older version can still access it by using the fully qualified path to the nested namespace that has that code.
        ...
        The following example shows two versions of an interface, each in a nested namespace.The v_20 namespace has some modification from the v_10 interfaceand is marked as inline.Client code that uses the new libraryand calls Contoso::Funcs::Add will invoke the v_20 version.Code that attempts to call Contoso::Funcs::Divide will now get a compile time error.If they really need that function, they can still access the v_10 version by explicitly calling Contoso::v_10::Funcs::Divide.
